...
from api_tools import detect_api_intent, get_crypto_price, get_news, get_weather
...
def run_api_check(
    ...
    *,
    ...
    if api_results:
        api_context = "\n".join(api_results)
        synthesis_prompt = (
            "Based on this real-time data, answer the user's question naturally.\n\n"
            f"Real-time data:\n{api_context}\n\n"
            f"User question: {prompt}\n\n"
            "Provide a clear, concise answer using the freshest data."
        )
        try:
            return ask_local(
                synthesis_prompt,
    ...
    fallback_prompt = (
        f"{prompt}\n\n(Note: Real-time data APIs were unavailable, responding with general knowledge.)"
    )
    return ask_local(
        fallback_prompt,
        system_prompt=system_prompt,
