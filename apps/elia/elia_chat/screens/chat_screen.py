...
class ChatScreen(Screen[None]):
    ...
    @on(Chat.RoutingStatusUpdate)
    def update_routing_status(self, event: Chat.RoutingStatusUpdate) -> None:
        """Update status message based on SEVEN routing progress."""
        response_status = self.query_one(ResponseStatus)
        response_status.set_status(event.status)
        response_status.display = True
