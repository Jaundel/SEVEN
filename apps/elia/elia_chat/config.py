...
def get_builtin_models() -> list[EliaChatModel]:
    """Return the single SEVEN router entry exposed in the UI."""
    return [
        EliaChatModel(
            id="seven-router",
            name="seven-router",
            display_name="SEVEN Router",
            provider="SEVEN",
            product="Hybrid local/cloud",
            description=(
                "Routes each prompt through SEVEN's heuristics, local model, "
                "real-time API tools, and OpenAI fallback only when required."
            ),
            temperature=0.3,
        )
    ]
...
class LaunchConfig(BaseModel):
    ...
    default_model: str = Field(default="seven-router")
    """The ID or name of the default model."""
    system_prompt: str = Field(
        default=os.getenv("ELIA_SYSTEM_PROMPT", get_system_prompt_local())
    )
